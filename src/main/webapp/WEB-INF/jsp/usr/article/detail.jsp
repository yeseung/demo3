  ...
  <div class="container mx-auto px-3">
    ...
    <div class="btns">
      ...
      <c:if test="${article.extra__actorCanModify}">
        <a class="btn btn-link" href="../article/modify?id=${article.id}&replaceUri=${rq.encodedCurrentUri}">게시판 수정</a>
      </c:if>
      <c:if test="${article.extra__actorCanDelete}">
        <a class="btn btn-link" onclick="if ( confirm('정말 삭제하시겠습니까?') == false ) return false;" href="../article/doDelete?id=${article.id}&replaceUri=${rq.encodedCurrentUri}">게시판 삭제</a>
      </c:if>
    </div>
  </div>
</section>
